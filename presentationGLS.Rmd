...
#### \ \ \ \ \ 2. We would like to **Improve Geographical predictions** by modeling the \ \ \ \ \ \ \ \ \ dependence structure.
...
**How do Geographical predictions look like?**
...
## Demonstration: Building a Covariance matrix
...
## Building a Kronecker covariance matrix
...
m <- leaflet() %>%
  addTiles() %>%
  addCircleMarkers(lng=coord$long.stn, lat=coord$lat.stn, popup=coord$stn, radius = 5)
m
```
...
Sparsity threshold can be determined in different ways, e.g. parametricly.
...
We would like to model the PM residual terms within an <span style="color:#FF4500"> **Autoregressive Moving Average** (ARMA) </span> model:

An ARMA(p,q) process is defined as follow:

$$ \large e_t = c + \varepsilon_t + \sum_{i=1}^p \rho_i e_{t-i} + \sum_{i=1}^q \theta_i \varepsilon_{t-i}$$
where:
...
**Note**: an ARMA model is appropriate only for **stationary** series!
...
This is a good point to check the **stationarity of the residuals**. A significant trend and seasonal component would indicate that the process is not stationary.

<br/>

We will not want to see here **neither trend nor seasonal effect** as it means that we missed them in in the fixed effects estimation.

<br/>

Here it seems that there is no significant trend and seasonal effects. To be sure, use **Unit root tests**.
...
We find the **order of the ARMA model** and its suitable parameters using AIC/BIC.
...
A **low ordered** ARMA may suggest we were able to **capture** most most of the dependence with **fixed effects**
...
**Note**: This structure recognizes all types of dependencies: Spatial, Temporal and Spatio-temporal
...
The degree of Sparsity here is 97%. 
